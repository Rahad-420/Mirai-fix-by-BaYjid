...
module.exports.run = async({api,event,args}) => {
  ...
  {body:`———»ADMIN BOT«———\n
  ❯ Name: 👑 一 ཐི༏ཋྀpiŋik BAYJID ཐི༏ཋྀ一 ℂℍ𝕆𝕎𝔻ℍ𝕌ℝ𝕐 👑
\n❯ Facebook: https://www.facebook.com/100065506668822 \n❯ Thanks for using ${global.config.BOTNAME} bot`,
    attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID, () => 
    fs.unlinkSync(__dirname + "/cache/1.png"));  
      return request(encodeURI(`https://i.postimg.cc/4dX0Kfnr/20230330-192553.png`)).pipe(
...
if (args[0] == "user") { 
    ...
    var gender = sex == 2 ? "Male" : sex == 1 ? "Female" : "info user";
    ...
    var gender = sex == 2 ? "Name" : sex == 1 ? "Female" : "info user";
    var callback = () => api.sendMessage({body:`Name: ${name}` + `\nPersonal URL: ${url}` + `\nUser name: ${sn}\nUID: ${args[1]}\nGender: ${gender}\nMake friends with bots: ${b}`,attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID);   
